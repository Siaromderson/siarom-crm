...
export default async function Page() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold title-grad">Notas Fiscais</h1>
        <p className="text-sm text-slate-500 dark:text-neutral-400">{arquivos.length} NF(s) arquivada(s)</p>
      </div>
      <GlassCard>
        <ArquivosList arquivos={arquivos} ownerLabels={ownerLabels} />
      </GlassCard>
      <div className="text-xs text-slate-400">Para anexar uma NF, abra o cliente ou projeto correspondente e use a aba <b>Arquivos</b>.</div>
    </div>
  );
}
